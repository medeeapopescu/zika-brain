...
def combine_gene_count(input, output):
  ...
  samplename = [x.strip() for x in input] # order same as input
  df = pd.read_table(samplename[0], header=None,index_col=0,names=[samplename[0]])
  
  
  for i in range(1,len(samplename)):
    print(i)
    df_n=pd.read_table(samplename[i], header=None,index_col=0,names=[samplename[i]])
    df=df.merge(df_n, how="outer", left_index=True, right_index=True)
    print(df.shape)
